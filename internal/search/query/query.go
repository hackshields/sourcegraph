...
// This has return type `step`.
func WithGlobbing(enabled bool) step {
	step := func(nodes []Node) ([]Node, error) {
		if enabled {
			nodes, err = mapGlobToRegex(nodes)
			if err != nil {
				return nil, err
			}
		}
...
func Pipeline(in string, parser ..., transformation /*with globbing */ ...,  satisfies ...) (Plan, error) {
